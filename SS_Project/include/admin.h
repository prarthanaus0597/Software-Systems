...
bool modify_customer_info(int connFD)
{
    ...
    case 4:
        // UPDATE login id
        writeBytes = write(connFD, "\nWhat's the updated value for LoginId ?: ", strlen("What's the updated value for LoginId ?:"));
        if (writeBytes == -1)
        {
            perror("Error while writing ADMIN_MOD_CUSTOMER_NEW_NAME message to client!");
            return false;
    ...
    case 5:
        // UPDATE password
        writeBytes = write(connFD, "\nWhat's the updated value for Password ?: ", strlen("What's the updated value for Password ?:"));
        if (writeBytes == -1)
        {
            perror("Error while writing ADMIN_MOD_CUSTOMER_NEW_NAME message to client!");
            return false;
